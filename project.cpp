...
int main()
{
	...
	cin>>n;	
	cout<<"\n\nEnter the arrival time of each Student_process: ";
	for(i=0;i<n;i++)
	{
		cout<<"\nEnter the arrival time of Student"<<i+1<<": ";
		cin>>s[i].arrival_time;
		s[i].student_id = i+1;
	}

	cout<<"\n\n Enter the burst time of each Student_process: ";
	for(i=0;i<n;i++)
	{
		cout<<"\nEnter the burst time of Student"<<i+1<<": ";
		cin>>s[i].burst_time;
	}
	sort(s, s+n, process_sort_student);
	
	cout<<"\n\n Student_ID"<<"\t"<<"ARRIVAL TIME"<<"\t"<<"BURST TIME"<<endl;
	for(i=0;i<n;i++)
	{
    ...
    if(burst_time1[count]<=time_quantum && burst_time1[count]>0) 
    { 
    ...
    { 
      burst_time1[count]=burst_time1[count]-time_quantum; 
      time+=time_quantum; 
    } 
  ...
  cout<<"\nAverage Query Time: "<<(wait_time)/n<<endl; 
